// src/components/TreeViewAdmin.tsx
...
interface TreeViewAdminProps {
  ...
  onAddRelative: (parent: Member, type: 'child' | 'spouse') => void;
  onManageRole: (m: Member, isAssigned: boolean) => void;
}

const TreeViewAdmin: React.FC<TreeViewAdminProps> = ({ 
  members, config, currentUserRole, allowedRootId, onEdit, onAddRelative, onManageRole 
}) => {
  ...
  // THUẬT TOÁN RANH GIỚI MOD: Tìm tất cả con cháu của allowedRootId
  const allowedIds = useMemo(() => {
    if (currentUserRole === 'super' || !allowedRootId) return null; 
    
    const allowed = new Set<string>();
    allowed.add(allowedRootId);

    let currentLevel = [allowedRootId];
    while (currentLevel.length > 0) {
      const nextLevel: string[] = [];
      for (const id of currentLevel) {
        const children = members.filter(m => m.parents?.fatherId === id || m.parents?.motherId === id);
  ...
  const renderNode = (nodeId: string) => {
    ...
    // Kiểm tra xem thẻ này đã được cấp quyền Mod chưa
    const isAssignedMod = config?.permissions?.mods 
      ? Object.values(config.permissions.mods).some((mod: any) => mod.rootId === person.id)
      : false;
          ...
          <div className="inline-flex items-center justify-center relative py-4">
             <div className={`flex items-center gap-0 ${person.gender === 'F' ? 'flex-row-reverse' : 'flex-row'}`}>
                <MemberCardBase 
                  ...
                  renderExtraButtons={
                    isAllowed && (
                      <div className="bg-stone-100 border-t border-stone-200 py-1.5 px-2 flex justify-center items-center gap-2">
                        <button onClick={() => onEdit(person)} className="hover:scale-125 transition-transform" title="Chỉnh sửa">📝</button>
                        <span className="text-stone-300">|</span>
                        <button onClick={() => onAddRelative(person, 'spouse')} className="hover:scale-125 transition-transform" title="Thêm Vợ/Chồng">
                          {person.gender === 'M' ? '➕👧' : '➕👶'}
                        </button>
                        ...
                        {currentUserRole === 'super' && (
                          <>
                            <span className="text-stone-300">|</span>
                            <button onClick={() => onManageRole(person, isAssignedMod)} className="hover:scale-125 transition-transform" title="Phân quyền">
                              {isAssignedMod ? '❌' : '⚡'}
                            </button>
                          </>
                        )}
      ...
      >
        <TransformComponent wrapperStyle={{ width: "100%", height: "100%" }}>
          <div className="p-[500px] min-w-max relative">
            {/* PHẪU THUẬT: Thước đo Số Đời - Phiên bản chống lệch tích tụ */}
            {!logic.focusId && (
              <div 
                ...
                */}
                {Array.from({ length: logic.maxGeneration + 1 }).map((_, idx) => (
                  <div 
                    ...
                    style={{ 
                      height: '345px', // Con số vàng khớp với (285px thẻ + 60px padding)
